...
namespace CustomerApp.Services
{
    public class FormsNavigationService : INavigationService
    {
        ...
        public Page SetPage(string rootPageKey)
        {
            var rootPage = GetPage(rootPageKey);
            var mainPage = new NavigationPage(rootPage);
            _navigationPageStack.Push(mainPage);
        ...
        public Page PushNewPage(string PageKey)
        {
            var NextPage = GetPage(PageKey);
            var NewPage = new NavigationPage(NextPage);
            _navigationPageStack.Push(NewPage);
            return NewPage;
        }
        ...
        public async Task NavigateAsyncWithLoad(string pageKey, object parameter,int delayTime, bool animated = true)
        {
            Page page;
            ...
            using (userDialogs.Loading("Loading...", null, null, true, MaskType.Black))
            {
                ...
                page = GetPage(pageKey, parameter);
            }
            await CurrentNavigationPage.Navigation.PushAsync(page, animated);
        }
